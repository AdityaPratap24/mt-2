        ...
        if (receiver.current.value.length > 0 && mailBody.current.value.length > 0 && subject.current.value.length > 0) {
            ...
            try {
                let responce = await fetch(
                    `https://react-http-310b4-default-rtdb.firebaseio.com/expenses/${removeSpecialChar(receiver.current.value)}.json`,
                    {
                        method: 'POST',
                ...
                if (responce.ok) {
                    ...
                    try {
                        let responce = await fetch(
                            `https://react-http-310b4-default-rtdb.firebaseio.com/expenses/${user}.json`,
                            {
                                method: 'POST',
